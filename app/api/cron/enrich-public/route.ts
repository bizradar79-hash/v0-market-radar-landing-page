...
export async function GET(req: Request) {
  ...
  console.log('[cron/enrich-public] Starting enrichment loop...')
  ...
  while (iteration < MAX_ITERS) {
    ...
    try {
      ...
      console.log(`[cron/enrich-public] Iteration ${iteration}: processed=${data.processed} success=${data.enrichedSuccess} remaining=${remaining}`)
      if (remaining === 0 || (data.processed || 0) === 0) break
    } catch (e: any) {
      console.error(`[cron/enrich-public] Iteration ${iteration} failed:`, e?.message)
      break
    }
  }

  console.log(`[cron/enrich-public] Done: ${totalSuccess} enriched, ${totalFailed} failed in ${iteration} iterations`)
