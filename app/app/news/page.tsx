...
import { ExternalLink, Clock, TrendingUp, TrendingDown, Minus, Loader2, Newspaper, Sparkles } from "lucide-react"
  ...
  async function generateNews() {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast({
          title: "חדשות נוספו!",
          description: `נמצאו ${data.count || 0} כתבות חדשות`,
        })
      } else {
        toast({
          title: "שגיאה",
          description: data.error || "לא הצלחנו ליצור חדשות",
          variant: "destructive",
        })
      }
    } catch (error) {
      ...
      toast({
        title: "שגיאה",
        description: "אירעה שגיאה בעת יצירת החדשות",
        variant: "destructive",
      })
    } finally {
      setGenerating(false)
      ...
      {news.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <Button className="mt-4" onClick={generateNews} disabled={generating}>
              <Sparkles className="ml-2 h-4 w-4" />
              אסוף חדשות עם AI
            </Button>
          </CardContent>
